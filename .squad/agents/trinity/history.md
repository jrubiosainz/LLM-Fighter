...
### 2026-03-15: Complete Visual Overhaul — Fullscreen Arcade Layout (V4)
- **Fullscreen canvas layout**: Removed fixed 1400px container. Game container now fills `100vw × 100vh`. Header hidden — "LLM FIGHTER" title rendered as subtle 8px text in canvas HUD. Footer buttons positioned absolute, hidden during fight phases via `data-phase` CSS attribute.
- **Dynamic canvas scaling**: `resizeCanvas()` method sizes canvas to fill parent container using `devicePixelRatio`. Logical 800×450 game space preserved via `ctx.translate(offsetX, offsetY)` + `ctx.scale(renderScale)` wrapper in `render()`. All existing sprite/HUD/effect code operates unchanged in logical coordinates. Fires on init and `window.resize`.
- **Compact thought panels**: Side panels (250px each) replaced with horizontal `thought-strip` at bottom — two thin panels side-by-side, max 52px height. Model name shown as inline badge; removed P1/P2 headers and "THINKING:" labels. `updateThought()` now replaces content (not appends) for single-line latest-thought display.
- **Simplified thought formatting**: `formatThought()` in ai-controller.js now extracts only 🎯 ACTION and [STRATEGY:] — outputs one compact HTML line. Fallback: first 80 chars. Eliminated multi-line health/distance/pattern analysis from panel.
- **Phase-aware UI**: Renderer sets `data-phase` on `.game-container` each frame. CSS rules hide `.game-footer` during fighting/intro/round_end/match_end phases with opacity transition.
- **Character select enhanced**: VS bar shows `P1 [model] ⚡VS⚡ [model] P2` updated live. Model cards now have circular icon with first letter. 600ms VS reveal animation when both selected (grid fades, VS bar enlarges). Grid scrollable on small screens.
- **Canvas vignette**: `_drawCanvasVignette()` renders radial gradient (transparent center → 35% black edges) in logical space after all game drawing. CSS `.game-stage::after` vignette reduced from 0.45 to 0.3 opacity.
- **Scanlines reduced**: CRT overlay opacity dropped from 0.25 to 0.12 for less visual noise.
- **All V1-V3 features preserved**: Sprites, shadows, hitstop, super meter, particles, dust, energy trails, KO overlay, victory screen — all unchanged, running in the scaled logical space.
